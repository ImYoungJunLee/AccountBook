...
/*사용금액*//*
        ...
        indexColon = recvMessage.indexOf(":");
        indexWon = recvMessage.indexOf("원");//'원' 이라는 글자의 위치를 읽어냄
        useMoney = recvMessage.substring(indexWon - 5, indexWon);//'원' 위치를 기준으로 문자를 자름, "원" 은 빼기

        Log.d("useMoney", String.valueOf(useMoney));

        Log.d("useeeeeeeeeee", String.valueOf(useMoney.charAt(0)));

        Log.d("rec1", String.valueOf(recvMessage));

        //money2의 첫번째 인자가 공백이면,(ex ' '7500 인 경우로써 천원 대를 의미)
        if (useMoney.charAt(0) == ' ' || Character.toString(useMoney.charAt(0)) == System.getProperty("line.separator")) {
            useMoney = useMoney.substring(1, 5);//공백부분을 제외하고 값을 가져온다.
        }

        Log.d("useMoney2", String.valueOf(useMoney));

        useMoney = useMoney.replaceAll(" ", "");//money함수의 공백을 제거

        Log.d("useMoney3", String.valueOf(useMoney));

            */
/*사용장소*//*

        recvMessage = recvMessage.replaceAll("\\s", "");//문자메세지의 모든 공백,줄바꿈 을 없앰.
        Log.d("useMoney4", String.valueOf(recvMessage));

        indexWon = recvMessage.indexOf("원");//'원'의 위치를 읽어냄
        useMoneyPlace = recvMessage.substring(indexWon + 1, recvMessage.length() - 2);//'원' 다음부터 국민카드 문자열의 마지막 두글자 제외('사용'제외) 하면 카드를 사용한 장소 읽음
        /*/
/*****************************************국민 끝********************************************
